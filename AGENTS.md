...
- Provider authentication includes two-factor verification after a valid password or connected Notes+ first factor. Verification uses six individual numeric fields and the local prototype clearly discloses that any complete six-digit code is accepted. Provider Settings displays a dummy QR code for authenticator setup; turning 2FA off requires a six-digit code but preserves the authenticator setup, and turning it on again requires another six-digit code from that previously configured setup.
...
- A Provider clarification request must include enough patient and encounter context to answer safely: reason for visit, source-by-source conflict evidence with recency, the affected draft section, an expandable read-only full encounter, structured answer choices, and an explicit Unable to confirm route. It must not look ready to sign while clarification is open.
- Provider clarification is optimized for older clinicians: use plain step-by-step language, the existing SAGE typography scale with the saved text-size preference, the same gray content canvas and white-card treatment as the full note, generous single-column answer targets, progressive disclosure for both conflicting sources and the draft/full note, and one obvious final action. Other answers support both manual typing and inline voice recording that becomes an editable transcript before submission.
