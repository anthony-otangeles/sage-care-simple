...
- Provider authentication includes two-factor verification after a valid password or connected Notes+ first factor. The local prototype clearly discloses that any non-empty verification code is accepted, keeps the workspace locked until a code is submitted, and provides a persisted authenticator-app setup/reconfiguration flow in Provider Settings.
